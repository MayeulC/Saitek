...
class T_communication:
    ...
    def getHandle(self):
        try:
            self.handle = self.context.openByVendorIDAndProductID(0x06a3,0x0cfa)
            self.hasHandle=1
        except:
            self.hasHandle=0
    ...
    def sendDpi(self, dpi):
        ...
        for mode in range(0,4):
            for axis in range(0,2):
                ...
                self.ctrl_value+=0x0001*int((dpi.getDpiData(mode,axis)-100)/25)
                self.controlWrite()
...
class T_data(QWidget):
    ...
    def displayResults(self):
        ...
        if self.dpiMode != self.lastDpiMode:
            self.tabs.setCurrentIndex(self.dpiMode-1) # dpiMode starts at 1
